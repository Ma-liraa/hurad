...
export function Trigger({ name, description, link, color }: ProjectProps) {
  return (
    <MorphingDialogTrigger
      style={{ borderRadius: "24px" }}
      className="group relative flex cursor-pointer flex-col items-start overflow-hidden rounded-3xl border border-zinc-200 bg-zinc-50 p-6 transition-all duration-500 hover:border-zinc-300 hover:bg-white hover:shadow-lg dark:border-zinc-800/50 dark:bg-zinc-900/50 dark:hover:border-zinc-700 dark:hover:bg-zinc-800"
    >
      <div
      ...
      <div className="relative z-10 mt-6 flex flex-col items-start">
        ...
        <MorphingDialogSubtitle className="mt-2 line-clamp-2 flex text-start text-sm leading-relaxed text-zinc-600 group-hover:text-zinc-900 dark:text-zinc-400 dark:group-hover:text-zinc-300">
          {description}
        </MorphingDialogSubtitle>
...
export function DialogContainer({
  ...
  return (
    <MorphingDialogContainer>
      <MorphingDialogContent
        className="relative flex h-auto w-[90%] max-w-3xl flex-col overflow-hidden rounded-3xl border border-zinc-200 bg-zinc-50 shadow-2xl dark:border-zinc-800 dark:bg-zinc-950"
        style={{ borderRadius: "24px" }}
      >
        <div className="flex h-[85vh] flex-col overflow-y-auto bg-zinc-50 md:h-auto dark:bg-zinc-950">
          <div className="relative w-full bg-zinc-100 dark:bg-zinc-900">
            {/* چک کردن وجود گالری قبل از رندر */}
          ...
          <div className="p-6 md:p-8">
            ...
            <div className="mt-6 space-y-4 text-justify text-base leading-7 text-zinc-700 dark:text-zinc-300">
              <p>{description}</p>
              <p>
                This project demonstrates advanced usage of React and Next.js.
                It includes responsive design, optimized performance, and modern
                UI/UX principles.
              </p>
            </div>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-8 flex w-full items-center justify-center gap-2 rounded-xl bg-zinc-900 px-4 py-3 text-sm font-semibold text-white hover:bg-zinc-800 sm:hidden dark:bg-zinc-800 dark:text-zinc-100 dark:hover:bg-zinc-700"
            >
              Visit Project <ExternalLink className="h-4 w-4" />
            </a>
          </div>
        ...
        <MorphingDialogClose className="fixed top-4 right-4 z-50 rounded-full bg-white/50 p-2 text-zinc-600 backdrop-blur-sm hover:text-zinc-900 dark:bg-zinc-900/50 dark:text-zinc-400 dark:hover:text-zinc-100" />
      </MorphingDialogContent>
    </MorphingDialogContainer>
  );
}
